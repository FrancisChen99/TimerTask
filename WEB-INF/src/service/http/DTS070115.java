 ...
 * 
 */
public class DTS070115 extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		String log_title = "=====各種上傳計畫資料介接浩展資訊服務.";
		...
		try {
			...
			Data01 = _mdsDB.getDataArrayList(_mdsDB.exeQuery
					("select SET_NAME,SET_VALUE  from DIS_CONFIG where set_name in('SCA15Flag','SCA15Url','HznCID','SCA15String','SCA15Date')  order by SET_NAME"));
			for(int i=0;i<Data01.size();i++){
				...
				if(strData01.equals("SCA15Flag"))
					strFlag = (Data02.get(1)==null)?"":(String)Data02.get(1);
				else if(strData01.equals("SCA15Url"))
				...
				else if(strData01.equals("SCA15String"))
					strColumnName = (Data02.get(1)==null)?"":(String)Data02.get(1);
				else if(strData01.equals("SCA15Date"))
...
logger.info(log_title+"URL======="+strUrl+"&LDate="+LDate+"&CID="+strCityID);
			...
			//String Jsonres = send(strUrl+"&CID=04");
            //String Jsonres = "";
			String strData = "";
...
//寫入資料交換紀錄表DTS_RECORD
				this._mdsDB.exeUpdate("INSERT INTO DTS_RECORD (CITY,SEQ,KIND,UPDATETIME,RCOUNT,UCOUNT,LDATE,LUSER) VALUES ('"+strCityID+"',"+seq+",'NFASCA15','"+com.mds.mdsDateTime.getNowDateTime("yyyyMMddHHmmss")+"','0','0','"+com.mds.mdsDateTime.getNowDateTime("yyyyMMddHHmmss")+"','DT')");
...
//					strGeometry = "";
					...
					//for(int j=0;j<11;j++){
						...
						if(tycgData03.length>2)
							strtycgData03 = (tycgData03.length>1)?tycgData03[2]:"";	
						...
						strtycgData03 = (strtycgData03==null || strtycgData03.equals("null"))?"":strtycgData03;
						strtycgData01 = strtycgData01.toUpperCase();
						if(strColumnName.indexOf(","+strtycgData01+",")!=-1){
							if(strtycgData01.equals("TYPE")){
								sql01.append("TYPE_CODE,");          sql02.append("'"+strtycgData02+"',");
							}else if(strtycgData01.equals("TYPE_NAME")){
								sql01.append("TYPE_NAME,");          sql02.append("'"+strtycgData02+"',");
							}else if(strtycgData01.equals("LDATE")){
								sql01.append("BUILDDATE,");          sql02.append("'"+strtycgData02+"',");
							}else if(strtycgData01.equals("LOCID")){
								strLocid = strtycgData02;
								sql01.append(strtycgData01+",");          sql02.append("'"+strtycgData02+"',");
							}else if(strtycgData01.equals("PLNDATE")){
								sql01.append("PLNDATE,");          sql02.append("'"+strtycgData02+"',");
							}else if(strtycgData01.equals("FILENAME")){
								sql01.append("FILENAME,");          sql02.append("'"+strtycgData02+"',");
							}else if(strtycgData01.equals("FILEPATH")){
								sql01.append("FILEPATH,"); 
								if(tycgData03.length>2)
									sql02.append("'"+strtycgData02 +":"+  strtycgData03 +"',");		
								else 
...
//					sql01.append("GEOMETRY,");     sql02.append("getpoint("+X+","+Y+"),");
					sql01.append("LDATE,");     sql02.append("'"+com.mds.mdsDateTime.getNowDateTime("yyyyMMddHHmmss")+"',");
					sql01.append("LUSER");      sql02.append("'浩展'");
					
					
					
					sql = "INSERT INTO NFASCA15 ("+sql01.toString()+") VALUES ("+sql02.toString()+")";

					try{
						this._mdsDB.beginTran();
						//ID先刪除後新增
						this._mdsDB.exeUpdate("DELETE FROM NFASCA15 WHERE ID='"+strId+"'");
//						CID先刪除後新增
//						this._mdsDB.exeUpdate("DELETE FROM SCA15 WHERE LOCID='"+strLocid+"'");
				...
				//寫入資料交換紀錄表DTS_RECORD
//				this._mdsDB.exeUpdate("INSERT INTO DTS_RECORD VALUES ('"+strCityID+"',"+seq+",'SCA15','"+com.mds.mdsDateTime.getNowDateTime("yyyyMMddHHmmss")+"','"+tycgData01.length+"','"+a+"','"+com.mds.mdsDateTime.getNowDateTime("yyyyMMddHHmmss")+"','DT')");
				this._mdsDB.exeUpdate("INSERT INTO DTS_RECORD (CITY,SEQ,KIND,UPDATETIME,RCOUNT,UCOUNT,LDATE,LUSER) VALUES ('"+strCityID+"',"+seq+",'SCA15','"+com.mds.mdsDateTime.getNowDateTime("yyyyMMddHHmmss")+"','"+tycgData01.length+"','"+a+"','"+com.mds.mdsDateTime.getNowDateTime("yyyyMMddHHmmss")+"','DT')");
				if(!strStartDate.equals("")){
					this._mdsDB.exeUpdate("UPDATE DIS_CONFIG SET SET_VALUE='' WHERE SET_NAME='SCA15Date'");
				}
	...
	private String send (String strurl) throws Exception {
		...
		try {
			...
			conn.connect();
			logger.info("開始產生檔案");
			String strFileNamePath = getServletContext().getRealPath("/")+"download\\SCA15\\"+com.mds.mdsDateTime.getNowDateTime("yyyyMMdd")+".txt";
			File newFile = new File(strFileNamePath);
			if(newFile.exists()){
