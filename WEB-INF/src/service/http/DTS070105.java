 ...
 * 
 */
public class DTS070105 extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			Data01 = _mdsDB.getDataArrayList(_mdsDB.exeQuery
					("select SET_NAME,SET_VALUE  from DIS_CONFIG where set_name in('SCA05Flag','SCA05Url','HznCID','SCA05String','SCA05Date')  order by SET_NAME"));
			for(int i=0;i<Data01.size();i++){
				...
				if(strData01.equals("SCA05Flag"))
					strFlag = (Data02.get(1)==null)?"":(String)Data02.get(1);
				else if(strData01.equals("SCA05Url"))
				...
				else if(strData01.equals("SCA05String"))
					strColumnName = (Data02.get(1)==null)?"":(String)Data02.get(1);
				else if(strData01.equals("SCA05Date"))
...
//寫入資料交換紀錄表DTS_RECORD
				this._mdsDB.exeUpdate("INSERT INTO DTS_RECORD (CITY,SEQ,KIND,UPDATETIME,RCOUNT,UCOUNT,LDATE,LUSER) VALUES ('"+strCityID+"',"+seq+",'NFASCA05','"+com.mds.mdsDateTime.getNowDateTime("yyyyMMddHHmmss")+"','0','0','"+com.mds.mdsDateTime.getNowDateTime("yyyyMMddHHmmss")+"','DT')");
				...
				//for(int i=0;i<11;i++){
					...
					//for(int j=0;j<11;j++){
						...
						if(strColumnName.indexOf(","+strtycgData01+",")!=-1){
							if(strtycgData01.equals("BID")){
								strBid = strtycgData02;
...
//								strId = strtycgData02;
					...
					sql = "INSERT INTO NFASCA05 ("+sql01.toString()+") VALUES ("+sql02.toString()+")";

					try{
						this._mdsDB.beginTran();
						//BID先刪除後新增
						this._mdsDB.exeUpdate("DELETE FROM NFASCA05 WHERE BID='"+strBid+"'");
						//先刪除後新增
						//this._mdsDB.exeUpdate("DELETE FROM SCA05 WHERE LOCID='"+strLocid+"'");
...
logger.info("總筆數============"+(tycgData01.length)+"筆,成功寫入"+a+"筆");
				...
				//寫入資料交換紀錄表DTS_RECORD
				this._mdsDB.exeUpdate("INSERT INTO DTS_RECORD (CITY,SEQ,KIND,UPDATETIME,RCOUNT,UCOUNT,LDATE,LUSER) VALUES ('"+strCityID+"',"+seq+",'NFASCA05','"+com.mds.mdsDateTime.getNowDateTime("yyyyMMddHHmmss")+"','"+tycgData01.length+"','"+a+"','"+com.mds.mdsDateTime.getNowDateTime("yyyyMMddHHmmss")+"','DT')");
				if(!strStartDate.equals("")){
					this._mdsDB.exeUpdate("UPDATE DIS_CONFIG SET SET_VALUE='' WHERE SET_NAME='SCA05Date'");
				}
...
logger.info(log_title + "====="+ getErrorInfoFromException(e));
	...
	private String send (String strurl) throws Exception {
		...
		try {
			...
			conn.connect();
			logger.info("開始產生檔案");
			String strFileNamePath = getServletContext().getRealPath("/")+"download\\SCA05\\"+com.mds.mdsDateTime.getNowDateTime("yyyyMMdd")+".txt";
			File newFile = new File(strFileNamePath);
			if(newFile.exists()){
