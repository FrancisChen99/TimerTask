 ...
 * 
 */
public class DTS070114 extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		String log_title = "=====防火管理相關附檔(如防護計畫)介接浩展資訊服務.";
		...
		try {
			...
			Data01 = _mdsDB.getDataArrayList(_mdsDB.exeQuery("select SET_NAME,SET_VALUE  from DIS_CONFIG where set_name in('SCA14Flag','SCA14Url','HznCID','SCA14String','SCA14Date')  order by SET_NAME"));
			for(int i=0;i<Data01.size();i++){
				...
				if(strData01.equals("SCA14Flag"))
					strFlag = (Data02.get(1)==null)?"":(String)Data02.get(1);
				else if(strData01.equals("SCA14Url"))
				...
				else if(strData01.equals("SCA14String"))
					strColumnName = (Data02.get(1)==null)?"":(String)Data02.get(1);
				else if(strData01.equals("SCA14Date"))
					strStartDate = (Data02.get(1)==null)?"":(String)Data02.get(1);
			}
			if(!strFlag.equals("Y")){
				logger.info
				(log_title+"=====================================("+strCityID+")==該縣市設定檔預設為不做資料交換=====================================");
				return;
			}
...
//			String LDate = com.mds.mdsDateTime.getNowDateTime("20180101");//第一次補齊DB用
...
//寫入資料交換紀錄表DTS_RECORD
				this._mdsDB.exeUpdate("INSERT INTO DTS_RECORD (CITY,SEQ,KIND,UPDATETIME,RCOUNT,UCOUNT,LDATE,LUSER) VALUES ('"+strCityID+"',"+seq+",'NFASCA14','"+com.mds.mdsDateTime.getNowDateTime("yyyyMMddHHmmss")+"','0','0','"+com.mds.mdsDateTime.getNowDateTime("yyyyMMddHHmmss")+"','DT')");
				...
				//for(int i=0;i<11;i++){
					...
					//for(int j=0;j<11;j++){
						...
						if(strColumnName.indexOf(","+strtycgData01+",")!=-1){
							if(strtycgData01.equals("FILENAME")){
								...
								sql01.append("UPDATEDATE,");          sql02.append("'"+strtycgData02+"',");
							}else if(strtycgData01.equals("TYPE")){
								sql01.append("TYPE,");          	sql02.append("'"+strtycgData02+ "',");
...
//					sql01.append("GEOMETRY,");     sql02.append("getpoint("+X+","+Y+"),");
					...
					sql = "INSERT INTO NFASCA14 ("+sql01.toString()+") VALUES ("+sql02.toString()+")";			
					
					try{
						this._mdsDB.beginTran();						
						//ID先刪除後新增
						this._mdsDB.exeUpdate("DELETE FROM NFASCA14 WHERE ID='"+strId+"' and LOCID='"+strLocid+"' and CID='"+strCityID+"'");
				...
				//寫入資料交換紀錄表DTS_RECORD
				this._mdsDB.exeUpdate("INSERT INTO DTS_RECORD (CITY,SEQ,KIND,UPDATETIME,RCOUNT,UCOUNT,LDATE,LUSER) VALUES ('"+strCityID+"',"+seq+",'NFASCA14','"+com.mds.mdsDateTime.getNowDateTime("yyyyMMddHHmmss")+"','"+tycgData01.length+"','"+a+"','"+com.mds.mdsDateTime.getNowDateTime("yyyyMMddHHmmss")+"','DT')");
				if(!strStartDate.equals("")){
					this._mdsDB.exeUpdate("UPDATE DIS_CONFIG SET SET_VALUE='' WHERE SET_NAME='SCA14Date'");
				}
	...
	private String send (String strurl) throws Exception {
		...
		try {
			...
			conn.connect();
			logger.info("開始產生檔案");
			String strFileNamePath = getServletContext().getRealPath("/")+"download\\SCA14\\"+com.mds.mdsDateTime.getNowDateTime("yyyyMMdd")+".txt";
			File newFile = new File(strFileNamePath);
			if(newFile.exists()){
