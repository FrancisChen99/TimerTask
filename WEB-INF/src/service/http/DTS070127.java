 ...
 * 
 */
public class DTS070127 extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		String log_title = "=====建物會審勘資料介接浩展資訊服務.";
		...
		try {
			...
			Data01 = _mdsDB.getDataArrayList(_mdsDB.exeQuery
					("select SET_NAME,SET_VALUE  from DIS_CONFIG where set_name in('INM027FHFlag','INM027FHUrl','HznCID','INM027FHString','INM027Date')  order by SET_NAME"));
			for(int i=0;i<Data01.size();i++){
				...
				if(strData01.equals("INM027FHFlag"))
					strFlag = (Data02.get(1)==null)?"":(String)Data02.get(1);
				else if(strData01.equals("INM027FHUrl"))
				...
				else if(strData01.equals("INM027FHString"))
					strColumnName = (Data02.get(1)==null)?"":(String)Data02.get(1);
				else if(strData01.equals("INM027Date"))
					strStartDate = (Data02.get(1)==null)?"":(String)Data02.get(1);
			}
			//因為改撈會審刊資料，BID==>CASEID(存的欄位還是叫BID)
			strColumnName = strColumnName.replaceAll(",BID,", ",CASEID,");
			if(!strFlag.equals("Y")){
				logger.info
...
//寫入資料交換紀錄表DTS_RECORD
				this._mdsDB.exeUpdate("INSERT INTO DTS_RECORD (CITY,SEQ,KIND,UPDATETIME,RCOUNT,UCOUNT,LDATE,LUSER) VALUES ('"+strCityID+"',"+seq+",'NFAINM027','"+com.mds.mdsDateTime.getNowDateTime("yyyyMMddHHmmss")+"','0','0','"+com.mds.mdsDateTime.getNowDateTime("yyyyMMddHHmmss")+"','DT')");
				if(!strStartDate.equals("")){
					this._mdsDB.exeUpdate("UPDATE DIS_CONFIG SET SET_VALUE='' WHERE SET_NAME='INM027Date'");
				}
				...
				//for(int i=0;i<11;i++){
					...
					//for(int j=0;j<11;j++){
						...
						if(strColumnName.indexOf(","+strtycgData01+",")!=-1){
							if(strtycgData01.equals("ID")){
								...
								sql01.append("BID,");          sql02.append("'"+strtycgData02+"',");
							}else if(strtycgData01.equals("FILETYPE")){

								sql01.append("FILETYPE,");          sql02.append("'"+strtycgData02+"',");
					...
					sql = "INSERT INTO NFAINM027 ("+sql01.toString()+") VALUES ("+sql02.toString()+")";

					try{
						this._mdsDB.beginTran();
						//先刪除後新增
						this._mdsDB.exeUpdate("DELETE FROM NFAINM027 WHERE ID='"+strid+"'");
...
//寫入資料交換紀錄表DTS_RECORD
				this._mdsDB.exeUpdate("INSERT INTO DTS_RECORD (CITY,SEQ,KIND,UPDATETIME,RCOUNT,UCOUNT,LDATE,LUSER) VALUES ('"+strCityID+"',"+seq+",'NFAINM027','"+com.mds.mdsDateTime.getNowDateTime("yyyyMMddHHmmss")+"','"+tycgData01.length+"','"+a+"','"+com.mds.mdsDateTime.getNowDateTime("yyyyMMddHHmmss")+"','DT')");
...
logger.info(log_title + "====="+ getErrorInfoFromException(e));
	...
	private String send (String strurl) throws Exception {
		...
		try {
			...
			conn.connect();
			logger.info("開始產生檔案");
			String strFileNamePath = getServletContext().getRealPath("/")+"download\\INM027FH\\"+com.mds.mdsDateTime.getNowDateTime("yyyyMMdd")+".txt";
			File newFile = new File(strFileNamePath);
			if(newFile.exists()){
