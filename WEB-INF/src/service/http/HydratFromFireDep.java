 ...
 * 
 */
public class HydratFromFireDep extends HttpServlet {

	//private static final long serialVersionUID = 1L;
	//private final String USER_AGENT = "Mozilla/5.0";
	base.MdsDbExtend _mdsDB = base.ConnectionProvider.getMdsDb();

	private org.apache.log4j.Logger logger = org.apache.log4j.Logger.getLogger("HydratFromFireDep");
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		String log_title = "=====進入消防局水源資料介接浩展資訊服務.";
logger.info(log_title);	
		...
		try {
			...
			Data01 = _mdsDB.getDataArrayList(_mdsDB.exeQuery("select SET_NAME,SET_VALUE  from DIS_CONFIG where (set_name like 'HydrantFromHoriZon%' or set_name='CITY_NAME' or set_name='CITY_ID' or set_name='CITY')  order by SET_NAME"));
			for(int i=0;i<Data01.size();i++){
				...
				if(strData01.equals("HydrantFromHoriZon"))
					strFlag = (Data02.get(1)==null)?"":(String)Data02.get(1);
				else if(strData01.equals("HydrantFromHoriZonUrl"))
					strUrl = (Data02.get(1)==null)?"":(String)Data02.get(1);
				else if(strData01.equals("CITY"))
					strCity = (Data02.get(1)==null)?"":(String)Data02.get(1);
				else if(strData01.equals("CITY_ID"))
					strCityID = (Data02.get(1)==null)?"":(String)Data02.get(1);
			}
			if(!strFlag.equals("Y")){
logger.info(log_title+"=====================================該縣市設定檔預設為不做資料交換=====================================");
				return;
			}
			
			out = response.getWriter();
			Data01 = _mdsDB.getDataArrayList(_mdsDB.exeQuery("select SET_NAME,SET_VALUE  from DIS_CONFIG where set_name like '%SingleSignOn_For_119%' order by SET_NAME"));
			...
			String Jsonres = send(strUrl + strCity);
			String strData = "";
			File newFile = new File(Jsonres);
			if(newFile.exists()){//表示回傳資料完成
				java.io.FileReader fr=new java.io.FileReader(Jsonres);
...
logger.info("=====================沒有水源資料=====================");
				...
				strData = strData.substring(1,strData.length()-1);//去掉頭尾的{跟 }
			}
...
//System.out.println("tycgData01.length===="+tycgData01.length);
				...
				this._mdsDB.exeUpdate("TRUNCATE TABLE WSD01");
				int a = 0;
				for(int i=0;i<tycgData01.length;i++){
					tycgData01[i] = tycgData01[i].replaceAll("\"", "");
logger.info("tycgData01.["+i+"]===="+tycgData01[i]);	
					/*
					 * 縣市:彰化縣
					 * 水源類別:地上式消防栓
					 * 水源形式:自來水
					 * 地址:彰化縣員林鎮三多里中山路2段556巷33號
					 * 經度:120.57281310718
					 * 緯度:23.9730107074911
					 * 標示牌狀況:良好
					 * 水源現況:良好-良好*/
					...
					sql01.setLength(0);
					sql02.setLength(0);
					sql01.append("DEPTNO,");      sql02.append("'"+strCityID+"0000',");
					sql01.append("WSNO,");      sql02.append("'"+i+"',");
					...
					for(int j=0;j<tycgData02.length;j++){
						...
						if(strtycgData01.trim().equals("水源類別")){
							if(strtycgData02.indexOf("消防栓")!=-1){
								sql01.append("KINDNO,");      sql02.append("'消防栓',");
								sql01.append("TYPENO,");      sql02.append("'"+strtycgData02.replaceAll("消防栓", "")+"',");
							}else{
								sql01.append("KINDNO,");      sql02.append("'"+strtycgData02+"',");
							}
						}
						if(strtycgData01.trim().equals("水源形式") && sql01.toString().indexOf("TYPENO")==-1){
							if(strtycgData02.indexOf("自來水")==-1){
								sql01.append("TYPENO,");      sql02.append("'"+strtycgData02+"',");
							}
						}
						
						if(strtycgData01.trim().equals("地址")){
							sql01.append("ADDRESS,");      sql02.append("'"+strtycgData02+"',");
						}
						
						if(strtycgData01.trim().equals("標示牌狀況")){
							sql01.append("PLUGSTATUS,");      sql02.append("'"+strtycgData02+"',");
						}
						
						if(strtycgData01.trim().equals("水源現況")){
							sql01.append("WSSTATUS,");      sql02.append("'"+strtycgData02+"',");
						}
						
						if(strtycgData01.trim().equals("經度")){
							X = (strtycgData02.equals("") || strtycgData02==null ||strtycgData02.equals("null"))?"0":strtycgData02;
						}
						
						if(strtycgData01.trim().equals("緯度")){
							Y = (strtycgData02.equals("") || strtycgData02==null ||strtycgData02.equals("null"))?"0":strtycgData02;
						}
							
					}

					XY = this.Cal_lonlat_To_twd97(X+","+Y);
					String[] XYArr = XY.split(",");
					
					//組合剩下的X,Y,GEOMETRY
					if(!(XYArr[0].equals("0")) || !(XYArr[1].equals("0"))){
						sql01.append("X,");         sql02.append(XYArr[0]+",");
						sql01.append("Y,");         sql02.append(XYArr[1]+",");
						strGeometry = "getpoint("+XYArr[0]+","+XYArr[1]+")";
						sql01.append("GEOMETRY,");  sql02.append(strGeometry+",");
					}
					...
					sql = "INSERT INTO WSD01 ("+sql01.toString()+") VALUES ("+sql02.toString()+")";
...
logger.info(this.getErrorInfoFromException(e1));						
				...
				this._mdsDB.exeUpdate("INSERT INTO DTS_RECORD (CITY,SEQ,KIND,UPDATETIME,RCOUNT,UCOUNT,LDATE,LUSER) VALUES ('"+strCityID+"',"+seq+",'WSD01','"+com.mds.mdsDateTime.getNowDateTime("yyyyMMddHHmmss")+"','"+tycgData01.length+"','"+a+"','"+com.mds.mdsDateTime.getNowDateTime("yyyyMMddHHmmss")+"','DT')");
...
//System.out.println("strurl==========="+strurl);
			...
			conn.connect();
logger.info("開始產生檔案");
			String strFileNamePath = getServletContext().getRealPath("/")+"download\\Hydrant\\"+com.mds.mdsDateTime.getNowDateTime("yyyyMMdd")+".txt";
			File newFile = new File(strFileNamePath);
