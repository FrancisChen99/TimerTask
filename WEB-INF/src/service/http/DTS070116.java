 ...
 * 
 */
public class DTS070116 extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		String log_title = "=====甲乙種圖來源介接浩展資訊服務.";
		...
		try {
			...
			Data01 = _mdsDB.getDataArrayList(_mdsDB.exeQuery
					("select SET_NAME,SET_VALUE  from DIS_CONFIG where set_name in('SCA16Flag','SCA16Url','HznCID','SCA16String','SCA16Date')  order by SET_NAME"));
			for(int i=0;i<Data01.size();i++){
				...
				if(strData01.equals("SCA16Flag"))
					strFlag = (Data02.get(1)==null)?"":(String)Data02.get(1);
				else if(strData01.equals("SCA16Url"))
				...
				else if(strData01.equals("SCA16String"))
					strColumnName = (Data02.get(1)==null)?"":(String)Data02.get(1);
				else if(strData01.equals("SCA16Date"))
...
//寫入資料交換紀錄表DTS_RECORD
				this._mdsDB.exeUpdate("INSERT INTO DTS_RECORD (CITY,SEQ,KIND,UPDATETIME,RCOUNT,UCOUNT,LDATE,LUSER) VALUES ('"+strCityID+"',"+seq+",'NFASCA16','"+com.mds.mdsDateTime.getNowDateTime("yyyyMMddHHmmss")+"','0','0','"+com.mds.mdsDateTime.getNowDateTime("yyyyMMddHHmmss")+"','DT')");
				...
				//for(int i=0;i<11;i++){
					...
					sql = "INSERT INTO NFASCA16 ("+sql01.toString()+") VALUES ("+sql02.toString()+")";

					try{
						this._mdsDB.beginTran();
						//ID先刪除後新增
						this._mdsDB.exeUpdate("DELETE FROM NFASCA16 WHERE ID='"+strId+"'");
						//先刪除後新增
//						this._mdsDB.exeUpdate("DELETE FROM SCA16 WHERE LOCID='"+strLocid+"'");
				...
				//寫入資料交換紀錄表DTS_RECORD
				this._mdsDB.exeUpdate("INSERT INTO DTS_RECORD (CITY,SEQ,KIND,UPDATETIME,RCOUNT,UCOUNT,LDATE,LUSER) VALUES ('"+strCityID+"',"+seq+",'NFASCA16','"+com.mds.mdsDateTime.getNowDateTime("yyyyMMddHHmmss")+"','"+tycgData01.length+"','"+a+"','"+com.mds.mdsDateTime.getNowDateTime("yyyyMMddHHmmss")+"','DT')");
				//this._mdsDB.exeUpdate("INSERT INTO DTS_RECORD VALUES ('"+strCityID+"',"+seq+",'SCA16','"+com.mds.mdsDateTime.getNowDateTime("yyyyMMddHHmmss")+"','"+tycgData01.length+"','"+a+"','"+com.mds.mdsDateTime.getNowDateTime("yyyyMMddHHmmss")+"','DT')");
				if(!strStartDate.equals("")){
					this._mdsDB.exeUpdate("UPDATE DIS_CONFIG SET SET_VALUE='' WHERE SET_NAME='SCA16Date'");
				}
...
//System.out.println("strurl==========="+strurl);
			...
			conn.connect();
logger.info("開始產生檔案");
			String strFileNamePath = getServletContext().getRealPath("/")+"download\\SCA16\\"+com.mds.mdsDateTime.getNowDateTime("yyyyMMdd")+".txt";
			File newFile = new File(strFileNamePath);
			if(newFile.exists()){
