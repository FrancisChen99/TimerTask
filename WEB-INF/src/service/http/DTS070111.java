 ...
 * 
 */
public class DTS070111 extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		String log_title = "=====公共危險物品種類及數量資訊介接浩展資訊服務.";
logger.info(log_title);	
		...
		try {
			...
			Data01 = _mdsDB.getDataArrayList(_mdsDB.exeQuery
					("select SET_NAME,SET_VALUE  from DIS_CONFIG where set_name in ('SCA11Flag','SCA11Url','HznCID','SCA11String','SCA11Date')  order by SET_NAME"));
			for(int i=0;i<Data01.size();i++){
				...
				if(strData01.equals("SCA11Flag"))
					strFlag = (Data02.get(1)==null)?"":(String)Data02.get(1);
				else if(strData01.equals("SCA11Url"))
				...
				else if(strData01.equals("SCA11String"))
					strColumnName = (Data02.get(1)==null)?"":(String)Data02.get(1);
				else if(strData01.equals("SCA11Date"))
...
//寫入資料交換紀錄表DTS_RECORD
				this._mdsDB.exeUpdate("INSERT INTO DTS_RECORD (CITY,SEQ,KIND,UPDATETIME,RCOUNT,UCOUNT,LDATE,LUSER) VALUES ('"+strCityID+"',"+seq+",'NFASCA11','"+com.mds.mdsDateTime.getNowDateTime("yyyyMMddHHmmss")+"','0','0','"+com.mds.mdsDateTime.getNowDateTime("yyyyMMddHHmmss")+"','DT')");
...
//					tycgData01[i] = tycgData01[i].replaceAll("\"", "");
					...
					//for(int j=0;j<11;j++){
						tycgData02[j] = tycgData02[j].replaceAll("\"", "");
						...
						if(strColumnName.indexOf(","+strtycgData01+",")!=-1){
							if(strtycgData01.equals("NAME")){
								sql01.append("NAME,");  			sql02.append("'"+strtycgData02 +"',");		
							}else if(strtycgData01.equals("LOCTYPE")){
								sql01.append("LOCTYPE,");          sql02.append("'"+strtycgData02+"',");
							}else if(strtycgData01.equals("LOCID")){
								sql01.append("LOCID,");    sql02.append("'"+strtycgData02+"',");
							}else if(strtycgData01.equals("ID")){
								strId = strtycgData02 ;
								sql01.append("ID,");          		sql02.append("'"+strtycgData02+"',");
							}else if(strtycgData01.equals("MAXQTY")){
								MAXQTY = strtycgData02;
					...
					sql01.append("MEMO,");      sql02.append("'"+ MAXQTY + CODEDESC +"',");
					sql01.append("LDATE,");     sql02.append("'"+com.mds.mdsDateTime.getNowDateTime("yyyyMMddHHmmss")+"',");
					sql01.append("LUSER");      sql02.append("'浩展'");
					
					
					
					sql = "INSERT INTO NFASCA11 ("+sql01.toString()+") VALUES ("+sql02.toString()+")";

					try{
						this._mdsDB.beginTran();
						//ID先刪除後新增
						this._mdsDB.exeUpdate("DELETE FROM NFASCA11 WHERE ID='"+strId+"'");
				...
				//寫入資料交換紀錄表DTS_RECORD
				this._mdsDB.exeUpdate("INSERT INTO DTS_RECORD (CITY,SEQ,KIND,UPDATETIME,RCOUNT,UCOUNT,LDATE,LUSER) VALUES ('"+strCityID+"',"+seq+",'NFASCA11','"+com.mds.mdsDateTime.getNowDateTime("yyyyMMddHHmmss")+"','"+tycgData01.length+"','"+a+"','"+com.mds.mdsDateTime.getNowDateTime("yyyyMMddHHmmss")+"','DT')");
				if(!strStartDate.equals("")){
					this._mdsDB.exeUpdate("UPDATE DIS_CONFIG SET SET_VALUE='' WHERE SET_NAME='SCA11Date'");
				}
		} catch (Exception e) {
	...
	private String send (String strurl) throws Exception {
		...
		try {
			...
			conn.connect();
			logger.info("開始產生檔案");
			String strFileNamePath = getServletContext().getRealPath("/")+
					"download\\SCA11\\"+com.mds.mdsDateTime.getNowDateTime("yyyyMMdd")+".txt";
			File newFile = new File(strFileNamePath);
			if(newFile.exists()){
