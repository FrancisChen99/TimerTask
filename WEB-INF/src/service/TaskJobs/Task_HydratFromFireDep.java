...
public class Task_HydratFromFireDep implements Job {
	private org.apache.log4j.Logger logger = org.apache.log4j.Logger.getLogger("TTask");
	public void execute(JobExecutionContext arg0) throws JobExecutionException {
		String title = "水源資料交換程式服務";
		try{
logger.info("********"+title+"********");
			
logger.info("*******檢查"+title+"是否開啟及URL********");
			base.MdsDbExtend mdsDB = base.ConnectionProvider.getMdsDb();
			java.util.ArrayList Data = mdsDB.getDataArrayList(mdsDB.exeQuery("select SET_NAME,SET_VALUE from DIS_CONFIG where SET_NAME IN ('Task_HydratFromFireDep','Task_HydratFromFireDep_Flag')"));
			...
			for(int i=0;i<Data.size();i++){
				...
				if(strSet_Name.equals("Task_HydratFromFireDep"))
					strUrl = (TempData.get(1)==null?"":TempData.get(1).toString());
				else if(strSet_Name.equals("Task_HydratFromFireDep_Flag"))
