...
public class Task_DTS070101 implements Job{
...
logger.info("*******檢查"+title+"是否開啟及URL********");
		...
		java.util.ArrayList Data = mdsDB.getDataArrayList(mdsDB.exeQuery("select SET_NAME,SET_VALUE from DIS_CONFIG where SET_NAME IN ('Task_DTS070101_DOWNLOAD','Task_DTS070101_DOWNLOAD_Flag')"));
		...
		for(int i=0;i<Data.size();i++){
			...
			if(strSet_Name.equals("Task_DTS070101_DOWNLOAD"))
				strUrl = (TempData.get(1)==null?"":TempData.get(1).toString());
			else if(strSet_Name.equals("Task_DTS070101_DOWNLOAD_Flag"))
