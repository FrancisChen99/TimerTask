...
logger.info("*******檢查"+title+"是否開啟及URL********");
		...
		java.util.ArrayList Data = mdsDB.getDataArrayList(mdsDB.exeQuery("select SET_NAME,SET_VALUE from DIS_CONFIG where SET_NAME IN ('Task_DT_Case_KPI_Upload','Task_DT_Case_KPI_Upload_Flag')"));
		...
		for(int i=0;i<Data.size();i++){
			...
			if(strSet_Name.equals("Task_DT_Case_KPI_Upload"))
				strUrl = (TempData.get(1)==null?"":TempData.get(1).toString());
			else if(strSet_Name.equals("Task_DT_Case_KPI_Upload_Flag"))
